...
for item in "${list[@]}" ; do
  ...
  nohup /usr/bin/time -f "#bit, BTreePlusAlpha, n = $1, %e sec, %M KB" ./build/bit_rank_select.out -x BTreePlusAlpha -n $1 -q 1000000 >> "./log/bit_rank_select_my.log"
  ...
  nohup /usr/bin/time -f "#psum, BTreePlusAlpha, n = $1, %e sec, %M KB" ./build/prefix_sum.out -x BTreePlusAlpha -n $1 -q 1000000 >> "./log/prefix_sum_my.log"
  # nohup /usr/bin/time -f "#psum, DYNAMIC, n = $1, %e sec, %M KB" ./prefix_sum.out -x DYNAMIC -n $1 -q 1000000 >> "./log/prefix_sum_dynamic.log"

  nohup /usr/bin/time -f "#WT, BTreePlusAlpha, sigma = 7, n = $1, %e sec, %M KB" ./build/rank_select.out -x BTreePlusAlpha -n $1 -q 1000000 -a 7 >> "./log/rank_select_my_7.log"
  nohup /usr/bin/time -f "#WT, BTreePlusAlpha, sigma = 63, n = $1, %e sec, %M KB" ./build/rank_select.out -x BTreePlusAlpha -n $1 -q 1000000 -a 63 >> "./log/rank_select_my_63.log"
  nohup /usr/bin/time -f "#WT, BTreePlusAlpha, sigma = 254, n = $1, %e sec, %M KB" ./build/rank_select.out -x BTreePlusAlpha -n $1 -q 1000000 -a 254 >> "./log/rank_select_my_254.log"
