...
void equal_test(const stool::bptree::SimpleDynamicPrefixSum &bv, const std::vector<uint64_t> &naive_bv)
{
...
void access_test(uint64_t max_len, uint64_t max_value, uint64_t number_of_trials, uint64_t seed)
{
    ...
    for (uint64_t i = 0; i < number_of_trials; i++)
    {
        ...
        while (len < max_len)
        {
            ...
            stool::bptree::SimpleDynamicPrefixSum value_deque(values);
...
void push_and_pop_test(uint64_t max_len, uint64_t max_value, uint64_t number_of_trials, uint64_t seed, bool detail_check = true)
{
    ...
    stool::bptree::SimpleDynamicPrefixSum value_deque;
...
void random_test(uint64_t max_len, uint64_t max_value, uint64_t number_of_trials, uint64_t max_counter, uint64_t seed, bool detail_check = false)
{
    ...
    for (uint64_t i = 0; i < number_of_trials; i++)
    {
        ...
        stool::bptree::SimpleDynamicPrefixSum value_deque(seq);
    ...
    for (uint64_t num = 16; num < max_element_count; num *= 2)
    {
        ...
        for (uint64_t x = 0; x < trial_count; x++)
        {
...

            {
                ...
                stool::bptree::SimpleDynamicPrefixSum::save(value_deque, os);
            }

            stool::bptree::SimpleDynamicPrefixSum value_deque2;

            {
                ...
                auto tmp = stool::bptree::SimpleDynamicPrefixSum::build_from_data(ifs);
...
void load_write_bits_test(uint64_t max_element_count, uint64_t trial_count, [[maybe_unused]] bool detailed_check, uint64_t seed)
{
    ...
    for (uint64_t num = 16; num < max_element_count; num *= 2)
    {
        ...
        for (uint64_t x = 0; x < trial_count; x++)
        {
            ...
            stool::bptree::SimpleDynamicPrefixSum::save(value_deque, bytes, pos);

            pos = 0;

            stool::bptree::SimpleDynamicPrefixSum value_deque2 = stool::bptree::SimpleDynamicPrefixSum::build_from_data(bytes, pos);
